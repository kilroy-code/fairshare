...
function timeLimit(nKeysCreated = 1) { // Time to create a key set varies quite a bit (deliberately).
  return (nKeysCreated * 2e3) + 3e3;
}
 
describe("User management", function () {
  let authorizedMember, commonGroup;
  ...
  async function expectMember(userTag, groupTag, {isMember = true, groupActor = userTag, expectGroupData = true, expectUserData = true} = {}) {
  ...
  beforeAll(async function () {
    ...
    await Group.store(Group.empty,    {tag: commonGroup,      owner: commonGroup,      author: bootstrapUserTag});
    //await messages.store('start commonGroup', {tag: commonGroup, owner: commonGroup, author: bootstrapUserTag});
    
    authorizedMember = await User.create();
    ...
    await expectMember(authorizedMember, commonGroup);    
  ...
  it("creates/destroys user.", async function () {
    const userTag = await User.create();
    await expectMember(userTag, Group.communityTag);
  ...
  it("creates/destroys group.", async function () {
    const groupTag = await Group.create(authorizedMember);
    await expectMember(authorizedMember, groupTag);

    await Group.destroy(groupTag, authorizedMember);
    await expectMember(authorizedMember, groupTag, {isMember: false, expectGroupData: false});
  }, timeLimit(1));
  ...
  it("adds/removes user from group.", async function () {
    ...
    await expectMember(candidate, Group.communityTag);
    ...
    await expectMember(candidate, groupTag, {keyActor: authorizedMember});

    await Group.deauthorizeUser(groupTag, candidate);    
    await User.abandonGroup(candidate, groupTag);
    await expectMember(candidate, groupTag, {isMember: false, groupActor: candidate});
